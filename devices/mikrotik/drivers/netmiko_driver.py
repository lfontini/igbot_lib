...
class MikrotikNetmiko(BaseMikrotik):
    ...
    def get_interfaces_strutured(self):
        ...
        for item in raw:
            ...
            result.append(Interface(id=clean["id"], name=clean["name"], status=clean.get("status", "Unknown"), type=clean["type"], mtu=clean["mtu"]))
    ...
    def get_ips_structured(self):
        ...
        for item in raw:
            clean = normalize(item)
            result.append(IpAddress(num=clean["num"], ip=clean["ip"], subnet=clean["subnet"], network=clean["network"], interface=clean["interface"]))
    ...
    def get_arp_structured(self):
        ...
        for item in raw:
            clean = normalize(item)
            result.append(Arp(num=clean["num"], ip=clean["ip_address"], mac_address=clean["mac_address"], interface=clean["interface"]))
        return result
